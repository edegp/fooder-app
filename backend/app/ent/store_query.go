...
type StoreQuery struct {
	...
	withNamedRecord map[string]*RecordQuery
...
func (sq *StoreQuery) QueryRecord() *RecordQuery {
...
func (sq *StoreQuery) Clone() *StoreQuery {
	...
	return &StoreQuery{
		...
		withRecord: sq.withRecord.Clone(),
...
func (sq *StoreQuery) WithRecord(opts ...func(*RecordQuery)) *StoreQuery {
	query := (&RecordClient{config: sq.config}).Query()
...
func (sq *StoreQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Store, error) {
	...
	if query := sq.withRecord; query != nil {
		if err := sq.loadRecord(ctx, query, nodes,
			func(n *Store) { n.Edges.Record = []*Record{} },
			func(n *Store, e *Record) { n.Edges.Record = append(n.Edges.Record, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range sq.withNamedRecord {
		if err := sq.loadRecord(ctx, query, nodes,
			func(n *Store) { n.appendNamedRecord(name) },
			func(n *Store, e *Record) { n.appendNamedRecord(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (sq *StoreQuery) loadRecord(ctx context.Context, query *RecordQuery, nodes []*Store, init func(*Store), assign func(*Store, *Record)) error {
	...
	query.Where(predicate.Record(func(s *sql.Selector) {
		s.Where(sql.InValues(store.RecordColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "place_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
// WithNamedRecord tells the query-builder to eager-load the nodes that are connected to the "record"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (sq *StoreQuery) WithNamedRecord(name string, opts ...func(*RecordQuery)) *StoreQuery {
	query := (&RecordClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if sq.withNamedRecord == nil {
		sq.withNamedRecord = make(map[string]*RecordQuery)
	}
	sq.withNamedRecord[name] = query
	return sq
}
