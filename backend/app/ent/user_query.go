...
func (uq *UserQuery) QueryRecord() *RecordQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withRecord: uq.withRecord.Clone(),
...
func (uq *UserQuery) WithRecord(opts ...func(*RecordQuery)) *UserQuery {
	query := (&RecordClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withRecord; query != nil {
		if err := uq.loadRecord(ctx, query, nodes,
			func(n *User) { n.Edges.Record = []*Record{} },
			func(n *User, e *Record) { n.Edges.Record = append(n.Edges.Record, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range uq.withNamedRecord {
		if err := uq.loadRecord(ctx, query, nodes,
			func(n *User) { n.appendNamedRecord(name) },
			func(n *User, e *Record) { n.appendNamedRecord(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadRecord(ctx context.Context, query *RecordQuery, nodes []*User, init func(*User), assign func(*User, *Record)) error {
...
func (uq *UserQuery) WithNamedRecord(name string, opts ...func(*RecordQuery)) *UserQuery {
	query := (&RecordClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if uq.withNamedRecord == nil {
		uq.withNamedRecord = make(map[string]*RecordQuery)
	}
	uq.withNamedRecord[name] = query
	return uq
}
