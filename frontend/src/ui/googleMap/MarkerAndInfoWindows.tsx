...
export const MarkerAndInfoWindows = memo(function InfoWindows() {
  ...
  const placeNames = makersLocation?.map((place: google.maps.places.PlaceResult) => [
    place.name,
    false
  ]) || [[]]
  ...
  const top5 = useMemo(
    () => makersLocation?.filter((place: google.maps.places.PlaceResult, i) => place.name && i < 5),
    [makersLocation]
  )
  ...
  return (
    <>
      {top5?.map((place: google.maps.places.PlaceResult, i) => {
